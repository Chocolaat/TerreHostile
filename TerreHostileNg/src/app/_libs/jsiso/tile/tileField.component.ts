...
export class JsIsoTileFieldComponent implements OnInit {
  ...
  function(ctx, mapWidth, mapHeight, mapLayout) {
    const _horizontalTileCache = {}; // Holds the canvas rendered tiles that are drawn via paths

    let title = '';
    let zeroIsBlank = false;
    let stackTiles = false;
    let stackTileGraphic = null;
    let drawX = 0;
    let drawY = 0;
    ...
    let heightOffset = 0;
    let heightShadows = null;
    let shadowSettings = null;

    let shadowDistance = null;

    let heightMapOnTop = false;

    let particleEffects = null;

    let curZoom = 1;
    let mouseUsed = false;
    let applyInteractions = false;
    let focusTilePosX = 0;
    let focusTilePosY = 0;
    let focusTilePosXBegin = 0;
    let focusTilePosXEnd = 0;
    ...
    let particleTiles = null;
    let particleMap = [];
    const particleMapHolder = [];

    let isometric = true;

    let tileImages = [];
    let tileImagesDictionary = [];
    ...
    function _drawHorizontalColorOverlay(xpos, ypos, graphicValue, stack, resizedTileHeight) {
      ...
      if (!isometric) {
        if (_horizontalTileCache[graphicValue] && _horizontalTileCache[graphicValue][curZoom]) {
          ctx.drawImage(_horizontalTileCache[graphicValue][curZoom], 0, 0,
            _horizontalTileCache[graphicValue][curZoom].width, _horizontalTileCache[graphicValue][curZoom].height, xpos,
            (ypos + ((tileHeight - resizedTileHeight) * curZoom)), (tileWidth * curZoom), (resizedTileHeight * curZoom));
        } else {
          if (!_horizontalTileCache[graphicValue]) {
            _horizontalTileCache[graphicValue] = {};
            _horizontalTileCache[graphicValue][curZoom] = {};
          }
          const holderCache = document.createElement('canvas'); // Set new Buffer element
          holderCache.width = tileWidth * curZoom;
          holderCache.height = tileWidth * curZoom;
          const ctxCache = holderCache.getContext('2d');
          ...
          ctx.drawImage(holderCache, 0, 0, holderCache.width, holderCache.height, xpos,
            (ypos + ((tileHeight - resizedTileHeight) * curZoom)), (tileWidth * curZoom),
            (resizedTileHeight * curZoom));
        }
        ...
        if (_horizontalTileCache[graphicValue] && _horizontalTileCache[graphicValue][curZoom]) {
          ctx.drawImage(_horizontalTileCache[graphicValue][curZoom], 0, 0,
            _horizontalTileCache[graphicValue][curZoom].width, _horizontalTileCache[graphicValue][curZoom].height,
            xpos, ypos + ((stack - 1) * tileOffset), (tileWidth * curZoom), (tileHeight * curZoom));
        } else {
          if (!_horizontalTileCache[graphicValue]) {
            _horizontalTileCache[graphicValue] = {};
            _horizontalTileCache[graphicValue][curZoom] = {};
          ...
          ctxCache.fill();
          _horizontalTileCache[graphicValue][curZoom] = holderCache;
          ctx.drawImage(holderCache, 0, 0, holderCache.width, holderCache.height,
            xpos, ypos + ((stack - 1) * tileOffset), (tileWidth * curZoom),
            (tileHeight * curZoom));
        }
      }
        ...
        if (!distanceLightingSettings || (distanceLightingSettings && distanceLighting < distanceLightingSettings.darkness)) {
          ...
          if (!particleMapHolder[i][j]) {
            if (particleEffects && particleEffects[particleMap[i][j]]) {
              ...
              // particleMapHolder[i][j] = new EffectLoader().getEffect(particleMap[i][j],
              // ctx, utils.range(0, mapHeight), utils.range(0, mapWidth));
            }
          }
          particleMapHolder[i][j].Draw(xpos, ypos + ((stack - 1) *
            (tileHeight - heightOffset - tileHeight)) * curZoom - (resizedTileHeight - tileHeight) * curZoom, curZoom);
        }
      }
    }
    ...
    function _draw(i, j, tileImageOverwite) {
      ...
      let resizedTileHeight;
      let stackGraphic = null;
      ...
      let distanceLighting = null;
      let distanceLightingSettings;
      let k = 0;
        ...
        if (lightMap) {
          ...
          for (let light = 0; light < lightMap.length; light++) {
            ...
            if (distanceLighting / (distanceLightingSettings.darkness *
              distanceLightingSettings.distance) > lightDist / (lightMap[light][2] * lightMap[light][3])) {
        ...
        if (!stackTiles) {
          ...
          if (!distanceLightingSettings || (distanceLightingSettings && distanceLighting < distanceLightingSettings.darkness)) {
            if (tileImageOverwite) {
              ...
              ctx.drawImage(tileImageOverwite, 0, 0, stackGraphic.width, stackGraphic.height, xpos,
                (ypos + ((tileHeight - resizedTileHeight) * curZoom)), (tileWidth * curZoom), (resizedTileHeight * curZoom));
            } else {
              ...
              if (alphaWhenFocusBehind && alphaWhenFocusBehind.apply === true) {
                if ((i === focusTilePosX + 1 && j === focusTilePosY + 1) ||
                (i === focusTilePosX && j === focusTilePosY + 1) ||
                (i === focusTilePosX + 1 && j === focusTilePosY)) {
                  if (alphaWhenFocusBehind.objectApplied &&
                    ((alphaWhenFocusBehind.objectApplied === null
                      || alphaWhenFocusBehind.objectApplied &&
                      (resizedTileHeight * curZoom) > alphaWhenFocusBehind.objectApplied.height * curZoom))) {
                    ctx.globalAlpha = 0.6;
                  }
                }
              }
              ...
              if (Number(graphicValue) >= 0) {
                ...
                if (stackGraphic !== undefined) {
                  ctx.drawImage(stackGraphic, 0, 0, stackGraphic.width, stackGraphic.height, xpos,
                    (ypos + ((tileHeight - resizedTileHeight) * curZoom)), (tileWidth * curZoom), (resizedTileHeight * curZoom));
                }
          ...
          if (heightMapOnTop) {
            ...
            if (!distanceLightingSettings || (distanceLightingSettings && distanceLighting < distanceLightingSettings.darkness)) {
              if (tileImageOverwite) {
                ...
                ctx.drawImage(tileImageOverwite, 0, 0, tileImageOverwite.width,
                  tileImageOverwite.height, xpos, ypos + ((stack - 1) *
                  (tileHeight - heightOffset - tileHeight)) * curZoom - (resizedTileHeight - tileHeight) *
                  curZoom, (tileWidth * curZoom), (resizedTileHeight * curZoom));
              } else {
                ...
                if (Number(graphicValue) >= 0) {
                  ...
                  if (alphaWhenFocusBehind && alphaWhenFocusBehind.apply === true) {
                    if ((i === focusTilePosX + 1 && j === focusTilePosY + 1) ||
                    (i === focusTilePosX && j === focusTilePosY + 1) || (i === focusTilePosX + 1 && j === focusTilePosY)) {
                      if (alphaWhenFocusBehind.objectApplied &&
                        (alphaWhenFocusBehind.objectApplied === null || (alphaWhenFocusBehind.objectApplied &&
                          (resizedTileHeight * curZoom) > alphaWhenFocusBehind.objectApplied.height * curZoom))) {
                        ctx.globalAlpha = 0.6;
                      }
                    }
                  }
                  ctx.drawImage(stackGraphic, 0, 0, stackGraphic.width, stackGraphic.height, xpos, ypos +
                    ((stack - 1) * (tileHeight - heightOffset - tileHeight)) * curZoom -
                    (resizedTileHeight - tileHeight) * curZoom, (tileWidth * curZoom), (resizedTileHeight * curZoom));
                  ctx.restore();
                } else if (graphicValue !== - 1) {
            ...
            for (k = 0; k <= stack; k++) {
              ...
              if (!distanceLightingSettings || (distanceLightingSettings && distanceLighting < distanceLightingSettings.darkness)) {
                if (tileImageOverwite) {
                  ...
                  ctx.drawImage(tileImageOverwite, 0, 0, tileImageOverwite.width,
                    tileImageOverwite.height, xpos, ypos +
                    (k * ((tileHeight - heightOffset - resizedTileHeight) * curZoom)),
                    (tileWidth * curZoom), (resizedTileHeight * curZoom));
                } else {
                  if (stackTileGraphic) {
                    if (k !== stack) {
                      ...
                      if (stackGraphic) {
                        ctx.drawImage(stackGraphic, 0, 0, stackGraphic.width, stackGraphic.height,
                          xpos, ypos + (k * ((tileHeight - heightOffset - resizedTileHeight) * curZoom)),
                          (tileWidth * curZoom), (resizedTileHeight * curZoom));
                      }
                      ...
                      if (Number(graphicValue) >= 0) {
                        ...
                        stackGraphic = tileImages[tileImagesDictionary[graphicValue]];
                        ctx.drawImage(stackGraphic, 0, 0, stackGraphic.width, stackGraphic.height,
                          xpos, ypos + ((k - 1) * ((tileHeight - heightOffset - resizedTileHeight) * curZoom)),
                          (tileWidth * curZoom), (stackGraphic.height / (stackGraphic.width / tileWidth) * curZoom));
                      } else if (graphicValue !== - 1) {
                        _drawHorizontalColorOverlay(xpos, ypos, graphicValue, k, resizedTileHeight);
                      }
                    ...
                    if (k === stack) {
                      ...
                      ctx.drawImage(stackGraphic, 0, 0, stackGraphic.width,
                        stackGraphic.height, xpos, ypos + (k * ((tileHeight - heightOffset - resizedTileHeight)
                        * curZoom)), (tileWidth * curZoom), (resizedTileHeight * curZoom));
                    }
                  }
                }
      ...
      if (heightShadows) {
        ...
        if (heightMap) {
          ...
          if (currStack < nextStack) {
            ...
            if (shadowSettings.horizontalColor) {
              if (!distanceLightingSettings || (distanceLighting < distanceLightingSettings.darkness)) {
                _drawHorizontalColorOverlay(shadowXpos, shadowYpos, (typeof shadowSettings.horizontalColor === 'string' ?
                shadowSettings.horizontalColor : shadowSettings.horizontalColor[i][j]), currStack, resizedTileHeight);
              }
            }
            // Apply Vertical shadow created from stacked tiles
            if (shadowSettings.verticalColor) {
              if (!distanceLightingSettings || (distanceLighting < distanceLightingSettings.darkness)) {
                _drawVeritcalColorOverlay(shadowXpos, shadowYpos, (typeof shadowSettings.verticalColor === 'string' ?
                shadowSettings.verticalColor : shadowSettings.verticalColor[i][j]), currStack, nextStack,
                resizedTileHeight, shadowSettings);
              }
            }
          }
        } else {
          ...
          if (currStack > 0) {
            shadowXpos = (i - j) * (tileHeight * curZoom) + drawX;
            shadowYpos = (i + j) * (tileWidth / 4 * curZoom) + drawY;
            _drawHorizontalColorOverlay(shadowXpos, shadowYpos, (typeof shadowSettings.verticalColor === 'string' ?
            shadowSettings.verticalColor : shadowSettings.verticalColor[i][j]), k, resizedTileHeight);
          }
        }
      }
        ...
        if (distanceLightingSettings.color !== false) {
          ...
          if (distanceLighting < distanceLightingSettings.darkness) {
            ...
            if (stackGraphic !== undefined || (zeroIsBlank && stackGraphic !== 0)) {
              _drawHorizontalColorOverlay(xpos, ypos, ('(' +
              distanceLightingSettings.color + ',' + distanceLighting + ')'), k, resizedTileHeight);
            }
          }
        }
      }
      if (mouseUsed && applyInteractions) {
        if (((i >= focusTilePosXBegin && i <= focusTilePosXEnd) || (i <= focusTilePosXBegin && i >= focusTilePosXEnd))
        && ((j >= focusTilePosYBegin && j <= focusTilePosYEnd) || (j <= focusTilePosYBegin && j >= focusTilePosYEnd))) {
    ...
    function _tileInView(tileX, tileY) {
      let distanceLighting = Math.sqrt((Math.round(tileX - lightX) * Math.round(tileX - lightX))
      + (Math.round(tileY - lightY) * Math.round(tileY - lightY)));
      if (lightMap) {
        let lightDist = 0;
        // Calculate which light source is closest
        for (let light = 0; light < lightMap.length; light++) {
    ...
    function _flip(setting) {
      if (stackTiles) {
        heightMap = this.utils.flipTwoDArray(heightMap, setting);
      }
      if (particleTiles) {
        // -- particleMap = utils.flipTwoDArray(particleMap, setting);
      }
      mapLayout = this.utils.flipTwoDArray(mapLayout, setting);

    }

    function _rotate(setting) {
      if (stackTiles) {
        heightMap = this.utils.rotateTwoDArray(heightMap, setting);
      }
      if (particleTiles) {
        // -- particleMap = utils.rotateTwoDArray(particleMap, setting);
      }
      mapLayout = this.utils.rotateTwoDArray(mapLayout, setting);
    }
    ...
    return {
      ...
      toggleGraphicsHide: function (toggle) {
        if (tilesHide !== null) {
          this._hideGraphics(toggle);
        }
      },

      toggleHeightShadow: function (toggle) {
        if (heightShadows !== null) {
          this._applyHeightShadow(toggle);
        }
      },
