...
export class JsIsoParticlesParticleComponent implements OnInit {
  ...
  utils = new JsIsoUtilsComponent();
      ...
      Draw(context) {
        ...
        if (this.active) {
          ...
          if (this.drawdelay === -1 || this.age >= this.drawdelay) {

            // Determine alpha based on life

            const alpha = this.life > 1.0 ? 1 : this.life < 0.0 ? 0 : this.life;

            const rgbstr = 'rgba(' + this.r + ', ' + this.g + ', ' + this.b + ', ' + this.utils.roundTo(alpha, 1) + ')';

            const rgbbgstr = 'rgba(' +  Math.floor(this.r / 3) + ', '
            +  Math.floor(this.g / 3) +  ', ' +
            Math.floor(this.b / 3) +  ', 0)';
            ...
            if (Number(this.x) !== undefined &&  Number(this.y) !== undefined) {
              ...
              if (this.x > this.minxb || this.x < this.maxxb || this.y > this.minyb || this.y < this.maxyb) {
                const p = context.createRadialGradient(this.x, this.y, 0, this.x, this.y, this.radius);
